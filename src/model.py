...
class Model:
  ...
  def encode(self, data, training):
    with tf.variable_scope('neurocode', reuse=tf.AUTO_REUSE, values=[data]):
      ...
      x = self.deconv2d(x, 8, 3, 2, name='encode_2', training=training)
      x = self.deconv2d(x, 8, 3, 2, name='encode_3', training=training)
  ...
  def decode(self, image, training):
    with tf.variable_scope('neurocode', reuse=tf.AUTO_REUSE, values=[image]):
      ...
      x = self.conv2d(x, 8, 3, 2, name='decode_3', training=training)
      x = self.conv2d(x, 8, 3, 2, name='decode_2', training=training)
